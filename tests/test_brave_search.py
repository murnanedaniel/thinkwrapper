...
class TestBraveSearchAPI:
    ...
    @patch('app.services.requests.get')
    @patch.dict(os.environ, {'BRAVE_SEARCH_API_KEY': 'test-api-key'})
    def test_search_brave_generic_exception_without_fallback(self, mock_get, app_context):
...
class TestBraveSearchLogging:
    ...
    def test_search_brave_logs_request(self, mock_get, app_context, caplog):
        ...
        assert any('Brave Search Request' in record.message for record in caplog.records)
    ...
    def test_search_brave_logs_response(self, mock_get, app_context, caplog):
        ...
        assert any('Brave Search Response' in record.message for record in caplog.records)
    ...
    def test_search_brave_logs_missing_api_key(self, app_context, caplog):
        ...
        assert any('Brave Search API key not configured' in record.message for record in caplog.records)
    ...
    def test_search_brave_logs_api_errors(self, mock_get, app_context, caplog):
        ...
        assert any('Brave API returned status code 500' in record.message for record in caplog.records)

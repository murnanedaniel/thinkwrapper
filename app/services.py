...
def search_brave(query, count=10, fallback_to_mock=True):
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            if fallback_to_mock:
                current_app.logger.info("Falling back to mock search results due to API error")
                return _get_mock_search_results(query, count)
        ...
        if fallback_to_mock:
            current_app.logger.info("Falling back to mock search results due to timeout")
            return _get_mock_search_results(query, count)
        ...
        if fallback_to_mock:
            current_app.logger.info("Falling back to mock search results due to exception")
            return _get_mock_search_results(query, count)
